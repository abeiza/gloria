	...
	class Manage_Category extends CI_Controller{
		...
		function index(){
			...
			if(!$cek){
				...
				$category = $this->model_back->get_data('gocweb_category');
				$config['total_rows'] = $category->num_rows();
				$config['base_url'] = base_url()."index.php/backend/manage_category/index/";
				...
				$data['category_grid'] = $this->model_back->get_data_limit('gocweb_category',$limit,$offset);
				$data['limit'] = $limit;
		...
		function form_add(){
			...
			$this->load->view('back/others/left_side');
			$this->load->view('back/others/top-header');
			$this->load->view("back/category/form_add_view");
			$this->load->view('back/others/bottom');
		}
		...
		function add(){
			...
			if($this->form_validation->run() == false){
				...
				$data['category_description'] = $this->input->post('desc');
				$data['category_date_update'] = date("Y-m-d H:i:s");
				
				$cek = $this->db->query("select category_name from gocweb_category where category_name='".$data['category_name']."'");
				if($cek->num_rows() == 0){
					$add_data = $this->model_back->get_insert('gocweb_category',$data);
					...
					if(!$add_data){
						...
						Header("Location:".base_url()."index.php/backend/manage_category/add/");
					}else{
		...
		function form_edit($id){
			...
			$this->load->model('model_back');
			$category_read = $this->model_back->get_data_where('category_id',$id,'gocweb_category');
			foreach($category_read->result() as $db){
				$data['id'] = $db->category_id;
				$data['name'] = $db->category_name;
				$data['desc'] = $db->category_description;
			}
			$this->load->view('back/others/top');
			$this->load->view('back/others/left_side');
			$this->load->view('back/others/top-header');
			$this->load->view("back/category/form_edit_view",$data);
			$this->load->view('back/others/bottom');
		}
		...
		function edit($id){
			...
			if($this->form_validation->run() == false){
				...
				$edit_data = $this->model_back->get_update('gocweb_category',$data,'category_id',$id);
		...
		function delete($id){
			...
			$delete = $this->model_back->get_delete('gocweb_category','category_id',$id);
		...
		function search(){
			...
			$search = $this->db->query("select * from gocweb_category where category_name like '%".$this->session->userdata('search_category')."%' or category_description like '%".$this->session->userdata('search_category')."%'");
			...
			$data['category_grid'] = $this->db->query("select * from gocweb_category where category_name like '%".$this->session->userdata('search_category')."%' or category_description like '%".$this->session->userdata('search_category')."%' LIMIT ".$limit." OFFSET ".$offset."");
			$data['limit'] = $limit;
