	...
	class Setting extends CI_Controller{
		...
		function index(){
			...
			if(!$cek){
				...
				$this->load->view('back/setting/view',$data);
		...
		function update_act(){
			...
			if($this->form_validation->run() == false){
				...
				if(empty($_FILES['logo']['name'])){
					...
					$data['setting_tag_line'] = $this->input->post('tagline');
					$data['setting_desc'] = $this->input->post('desc');
					$data['setting_vision'] = $this->input->post('visi');
					$data['setting_mission'] = $this->input->post('misi');
					$data['setting_moto'] = $this->input->post('moto');
					$data['setting_date_update'] = date("Y-m-d H:s:i");
					
					$input = $this->model_back->get_update('gocweb_setting',$data,'setting_id','1');
					if(!$input){
