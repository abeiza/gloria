				...
				<div style="float:left;width:100%;height:11.5%;background:#fff;color:#787878;">
					<div style="margin:0px 20px;width:auto;">	
						<h2 style="text-transform:uppercase;"><i class="fa fa-ticket" style="margin-right:10px"></i>Page Category</h2>
					</div>
				</div>
				<!-- End of header -->
				<!-- Separator -->
							...
							<div style="background-color:#fff;border-radius:3px;padding:10px;margin:20px 0px;">
								<div style="margin-bottom:20px">
									...
									<div style="float:left;margin:10px 0px;">
										<a href="<?php echo base_url();?>index.php/backend/manage_category_page/add/"><button style="font-size:14px;padding:5px 10px;border:1px solid #ccc;list-style:none;color:#717171;margin:0px 2px;background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(#f8f8f8), to(#e9e9e9));
    background: -moz-linear-gradient(0% 0% 270deg,#f8f8f8, #e9e9e9);border-radius:3px;box-shadow: inset 0px 1px 0px rgba(255,255,255, .8), 0px 1px 3px rgba(0,0,0, .1);"><i class="fa fa-plus" style="margin-right:5px;"></i>Add Category</button></a>
									</div>
									<div style="float:left;margin:10px 0px;">
										<a href="<?php echo base_url();?>index.php/backend/manage_category_page"><button style="font-size:14px;padding:5px 10px;border:1px solid #ccc;list-style:none;color:#717171;margin:0px 2px;background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(#f8f8f8), to(#e9e9e9));
    background: -moz-linear-gradient(0% 0% 270deg,#f8f8f8, #e9e9e9);border-radius:3px;box-shadow: inset 0px 1px 0px rgba(255,255,255, .8), 0px 1px 3px rgba(0,0,0, .1);"><i class="fa fa-refresh" style="margin-right:5px;"></i>Refresh</button></a>
									...
									<?php echo form_open('backend/manage_category_page/search/');?>
    ...
    background: -moz-linear-gradient(0% 0% 270deg,#f8f8f8, #e9e9e9);border-radius:3px;box-shadow: inset 0px 1px 0px rgba(255,255,255, .8), 0px 1px 3px rgba(0,0,0, .1);">
											<td style="border-bottom:1px solid #ccc;padding:10px;font-weight:bold">Category Name</td>
											<td style="border-bottom:1px solid #ccc;padding:10px;font-weight:bold">Description</td>
											<td style="border-bottom:1px solid #ccc;padding:10px;font-weight:bold">Post</td>
										</tr>
									</thead>
									<tbody>
										<?php if($category_grid->num_rows() == 0){?>
											<tr>
												<td colspan="3">Data Categories is Empty</td>
											</tr>
										<?php }else{ 
												foreach($category_grid->result() as $db){
													echo "<tr>";
													...
													<div style='width:200px'>
															...
															<div style='float:left;width:70%'>
																<span style='width:100%;float:left'>".$db->page_category_name."</span>
																<span style='width:100%;float:left'><a style='margin-right:15px;font-size:12px;text-decoration:none;color:#666;' href='".base_url()."index.php/backend/manage_category_page/form_edit/".$db->page_category_id."'><i class='fa fa-edit' style='margin-right:5px;'></i>Edit</a> <a style='margin-right:15px;font-size:12px;text-decoration:none;color:#666;' href='".base_url()."index.php/backend/manage_category_page/delete/".$db->page_category_id."'><i class='fa fa-trash-o' style='margin-right:5px;'></i>Delete</a></span>
															</div>
													</div>
														</td>";
													echo "<td style='border-bottom:1px solid #ccc;padding:10px;'>".$db->page_category_description."</td>";
													$category_count = $this->db->query("select page_id from gocweb_page where page_category='".$db->page_category_id."'");
													echo "<td style='border-bottom:1px solid #ccc;padding:10px;'>".$category_count->num_rows()."</td>";
													echo "</tr>";
												}
										 }?>
									</tbody>
